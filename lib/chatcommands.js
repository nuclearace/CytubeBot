...
CHAT_HANDLERS.set('islive', async (bot, username, msg) => {
  ...
  const isLive = await bot.twitch.isChannelLive(channel);
  if (isLive) {
...
CHAT_HANDLERS.set('nonotifylive', async (bot, username, msg) => {
  ...
  const channel = await bot.twitch.getChannel(channelName);
  ...
  bot.firstTwitchChannelMonitorDone = false;
...
CHAT_HANDLERS.set('notifylive', async (bot, username, msg) => {
  ...
  const status = (await bot.twitch.isChannelLive(channel.name)) ? 'live' : 'not live';
  bot.firstTwitchChannelMonitorDone = false;
