...
CHAT_HANDLERS.set('roulette', async (bot, username, msg, fromIrc) => {
  ...
  const gambledPoints = gambledAmountMsg === 'all' ?
      currentPoints :
      parseInt(gambledAmountMsg, 10);
...
CHAT_HANDLERS.set('update', async (bot, username, msg, fromIrc) => {
  ...
  exec('git pull', (error, stdout, stderr) => {
    if (error) {
      errlog.log(`error running git pull: ${error}`);
    }
    if (stderr) {
      errlog.log(`error running git pull: ${stderr}`);
    }
    cytubelog.log(`Results of running git pull: ${stdout}`);
    if (error || stderr) {
      bot.sendChatMsg('Encountered error while updating, please check logs.');
      return;
    }
