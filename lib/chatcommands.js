...
var chatHandlers = {
	...
	"bump": function(bot, username, data) {
		var rank = utils.handle(bot, "getUser", username)["rank"]
		var permissionData = {
			permission: "B",
			name: username.toLowerCase()
		...
		var match = data.trim().match(/^(\-(user)|(title)) (\w*) ?(all|\d)?/i)

		if (match) {
			console.log(match)
			var bumpKind = match[2]
			var bumpData = match[4]
			var num = match[5]

			if (bumpKind === "user") {
				var uids = utils.handle(bot, "findVideosAddedByUser", bumpData).reverse()
				if (!num) {
					bot.sendMoveMedia(uids[0])
				} else if (num && num.toLowerCase() === "all") {
