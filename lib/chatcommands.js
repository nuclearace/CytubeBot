...
import {genericUIDLoop, getCurrentUnixTimestamp, getUser, kill, parseBumpData, parseDeleteData, parseForecastData, parseMediaLink, parseUserlimit, sendPMsWithRateLimit, sleep} from './utils.js';
...
// Unlisted command.
CHAT_HANDLERS.set('debuguserlist', (bot, username, msg) => {
  if (msg) {
...
CHAT_HANDLERS.set('join', async (bot, username, msg) => {
  ...
  bot.usersInRaffle.push(username);
});
...
CHAT_HANDLERS.set('logs', async (bot, username, msg) => {
  ...
  if (!(await bot.checkPermission(username, Rank.ADMIN, 'K'))) {
    bot.sendChatMsg(
        `${username} does not have permission to logs. FeelsWeirdMan`);
    return;
  }

  const logFileError = 'error';
  const logFileCytubeLog = 'cytubelog';
  const logFileSys = 'sys';
  const logFiles = [logFileError, logFileCytubeLog, logFileSys];

  const fileName = msg.split(' ')[0];
  if (fileName === '') {
    bot.sendPM(
        username, `Name of log must be provided. Example: $logs error first 5`);
    return;
  }
  ...
  if (direction === '') {
    bot.sendPM(
        username,
        `Direction to search must be provided. Example: $logs error last 10`);
    return;
  }
  ...
  if (numberOfLinesMsg === '') {
    bot.sendPM(
        username,
        `Number of lines must be provided. Example: $logs error last 10`);
    return;
  }
  ...
  if (isNaN(numberOfLines)) {
    bot.sendPM(
        username,
        `Failed to parse number of lines. Example: $logs error last 10`);
    return;
  }

  const lines = [];
  if (fileName === logFileError) {
    const data = forward ? await errlog.read(numberOfLines) :
                           await errlog.readReverse(numberOfLines);
    lines.push(...data);
  } else if (fileName === logFileCytubeLog) {
    const data = forward ? await cytubelog.read(numberOfLines) :
                           await cytubelog.readReverse(numberOfLines);
    lines.push(...data);
  } else if (fileName === logFileSys) {
    const data = forward ? await syslog.read(numberOfLines) :
                           await syslog.readReverse(numberOfLines);
    lines.push(...data);
  } else {
    bot.sendPM(
        username,
...
CHAT_HANDLERS.set('raffle', async (bot, username, msg) => {
  if (!(await bot.db.moduleIsEnabled('raffle'))) {
    bot.sendChatMsg(
        'Raffle module is disabled. To enable, use $module raffle on');
    return;
  }
  ...
  if (currentPoints === 0) {
    bot.sendChatMsg(
        `${username}: you don't have any points to raffle with Sadeg`);
    return;
  }
  ...
  setTimeout(async () => {
    ...
    if (bot.usersInRaffle.length === 0) {
      winner = username;
      bot.sendChatMsg('No-one joined the raffle...');
    } else {
      winner = bot.usersInRaffle[randomInt(bot.usersInRaffle.length)];
      bot.sendChatMsg(
          `${winner} won the raffle and receives ${points} points!`);
    }
...
CHAT_HANDLERS.set('quote', (bot, username, msg) => {
  bot.getQuote(msg);
});
...
CHAT_HANDLERS.set('wolfram', (bot, username, msg) => {
  if (!bot.wolfram) {
