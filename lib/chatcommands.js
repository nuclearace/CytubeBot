...
CHAT_HANDLERS.set('logs', async (bot, username, msg) => {
  ...
  if (fileName === logFileError) {
    const data =
        forward ? await errlog.read(numberOfLines) : await errlog.readReverse(numberOfLines);
    lines.push(...data);
  } else if (fileName === logFileCytubeLog) {
    const data =
        forward ? await cytubelog.read(numberOfLines) : await cytubelog.readReverse(numberOfLines);
    lines.push(...data);
  } else if (fileName === logFileSys) {
    const data =
        forward ? await syslog.read(numberOfLines) : await syslog.readReverse(numberOfLines);
    lines.push(...data);
  } else {
    bot.sendPm(
        username,
...
CHAT_HANDLERS.set('tempban', async (bot, username, msg) => {
  ...
  if (isNaN(lengthMins) || lengthMins < 0) {
    bot.sendChatMsg('Failed to parse temp ban length. Example: $tempban IP0G 5');
    return;
  }
