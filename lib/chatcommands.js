...
import {genericUIDLoop, getCurrentUnixTimestamp, getUser, parseBumpData, parseDeleteData, parseForecastData, parseMediaLink, parseUserlimit} from './utils.js';
...
const pointsHandler = async (bot, username, msg, fromIrc) => {
  if (fromIrc) {
    return;
  }
  const user = msg.trim() || username;
...
CHAT_HANDLERS.set('addpoints', async (bot, username, msg, fromIrc) => {
  ...
  const deltaMsg = msg.split(' ')[1].trim().toLowerCase();
...
CHAT_HANDLERS.set('roulette', async (bot, username, msg, fromIrc) => {
  ...
  const gambledPoints = parseInt(msg.split(' ')[0], 10);
  ...
  const win = Math.round(Math.random() * 100) > (100 - winPercentage);
  ...
  bot.sendChatMsg(
      ...
      `and now has ${newPoints} points! ${win ? 'PagMan' : 'Sadeg'}`);
});
