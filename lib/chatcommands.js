...
CHAT_HANDLERS.set('leaderboard', async (bot, username, msg) => {
  ...
  bot.sendChatMsg(`Points leaderboard ${start}-${end}:`);

  for (const [i, user] of page.entries()) {
    const rank = i + firstRank;
    bot.sendChatMsg(`#${rank} ${user.points} points: ${user.name}`);
  }

  if (page.length === pageSize) {
    bot.sendChatMsg(`For the next page, do \`$leaderboard ${pageNumber + 1}\``);
  }
});
