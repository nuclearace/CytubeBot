...
CHAT_HANDLERS.set('userlogs', async (bot, username, msg) => {
  ...
  if (msg.split(' ')[1] == null) {
    bot.sendChatMsg(`${username}'s logs in ${channel}'s channel: https://logs.ivr.fi/?channel=${channel}&username=${username}`);
  } else {
    const user = msg.split(' ')[1];
    bot.sendChatMsg(`${user}'s logs in ${channel}'s channel: https://logs.ivr.fi/?channel=${channel}&username=${user}`);
  }
});
