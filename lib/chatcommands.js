...
CHAT_HANDLERS.set('leaderboard', async (bot, username, msg, fromIrc) => {
  const pageSize = 5;
  const pageMsg = msg.split(' ')[0];
  const pageMsgParsed = pageMsg !== '' ? parseInt(pageMsg, 10) : 0;
  const pageNumber = !isNaN(pageMsgParsed) ? pageMsgParsed : 0;

  const page = await bot.db.getLeaderboardPage(pageNumber, pageSize);
  bot.sendChatMsg(
      `Points leaderboard ${1 + pageNumber}-${pageNumber + pageSize}:`);
  for (const [i, user] of page.entries()) {
    bot.sendChatMsg(`${i}: ${user.points} - ${user.name}`);
  }

  if (page.length === pageSize) {
    bot.sendChatMsg(
        `For the next page, do \`$leaderboard ${pageNumber + pageSize}\``);
  }
});
