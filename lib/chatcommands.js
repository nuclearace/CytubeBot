...
CHAT_HANDLERS.set('rngkick', async (bot, username, msg, fromIrc) => {
  ...
  if (length > (maxLengthMins * .75)) {
    emote = 'pepeLaugh Boot';
  } else if (length < (maxLengthMins * .25)) {
  ...
  bot.sendChatMsg(
      `RNG kicking ${targetUser} for ${length} minutes, ${emote}`, true);
  bot.sendChatMsg(`/kick ${targetUser}`, true);

  const end = getCurrentUnixTimestamp() + (length * 60);
