...
var chatHandlers = {
	...
	"ban": function(bot, username, data, fromIRC) {
		...
		var hasPermission = bot.checkPermission(username, 2, "N")
	...
	"duplicates": function(bot, username, data, fromIRC) {
		...
		if (!hasPermission || bot.playlist.length === 0)
			return
	...
	"kick": function(bot, username, data, fromIRC) {
		...
		var hasPermission = bot.checkPermission(username, 2, "I")
	...
	"settime": function(bot, username, data, fromIRC) {
		...
		var hasPermission = bot.checkPermission(username, 2, "T")
		...
		bot.sendAssignLeader(bot.username)

		var timesCheckedLeader = 0
		var checker = setInterval(function() {
			if (timesCheckedLeader > 10)
				clearInterval(checker)

			if (bot.isLeader) {
				bot.logger.syslog.log("!~~~! Setting time on video to: " + time)
				bot.sendMediaUpdate(time, false)
				bot.sendAssignLeader("")
				timesCheckedLeader = 10
			}
