...
CHAT_HANDLERS.set('delete', async (bot, username, msg) => {
  if (!(await bot.checkPermission(username, Rank.MOD, 'D'))) {
    bot.sendChatMsg(`${username} does not have permission to delete. FeelsWeirdMan`);
    return;
  }
...
CHAT_HANDLERS.set('logs', async (bot, username, msg) => {
  ...
  if (!(await bot.checkPermission(username, Rank.ADMIN, 'K'))) {
    bot.sendChatMsg(`${username} does not have permission to logs. FeelsWeirdMan`);
    return;
  }
...
CHAT_HANDLERS.set('rngtimeout', async (bot, username, msg) => {
  ...
  const lengthDesc = humanizeDuration(lengthSecs * 1000);
  bot.sendChatMsg(`RNG rng-timing-out ${targetUser} for ${lengthDesc} :tf: Boot`);
...
CHAT_HANDLERS.set('smp', async (bot, username, msg) => {
  ...
  if (gambledPoints < 0) {
    bot.sendChatMsg('nice try forcenCD');
    return;
  }
  ...
  if (sl1 === sl2 && sl1 === sl3) {
    if (sl1 === 'xqcL' || sl1 === 'PagMan' || sl1 === 'TriHard' || sl1 === 'klaiusGuraRickRoll' ||
        sl1 === 'FEELSWAYTOOGOOD' || sl1 === 'ip0gL') {
      win = (gambledPoints * 50);
    } else if (
        sl1 === 'monkaPog' || sl1 === 'halalChad' || sl1 === 'PagPls' || sl1 === 'CrabPls' ||
        sl1 === 'Okayeg' || sl1 === 'CaitlynS' || sl1 === 'veiO' || sl1 === 'mendoUA' ||
        sl1 === 'elisNom' || sl1 === 'mendoUWU' || sl1 === 'Bald1G' || sl1 === 'FeelsOkayMan') {
      win = (gambledPoints * 35);
    } else {
      win = (gambledPoints * 25);
    }
  }
  if (sl1 === sl2 && sl1 !== sl3 || sl1 === sl3 && sl1 !== sl2 || sl2 === sl3 && sl1 !== sl2) {
    if (sl1 === 'xqcL' || sl1 === 'PagMan' || sl1 === 'TriHard' || sl1 === 'klaiusGuraRickRoll' ||
        sl1 === 'FEELSWAYTOOGOOD' || sl1 === 'ip0gL' || sl2 === 'xqcL' || sl2 === 'PagMan' ||
        sl2 === 'TriHard' || sl2 === 'klaiusGuraRickRoll' || sl2 === 'FEELSWAYTOOGOOD' ||
        sl2 === 'ip0gL') {
      win = (gambledPoints * 25);
    } else if (
        sl1 === 'monkaPog' || sl1 === 'halalChad' || sl1 === 'PagPls' || sl1 === 'CrabPls' ||
        ...
        sl2 === 'elisNom' || sl2 === 'mendoUWU' || sl2 === 'Bald1G' || sl2 === 'FeelsOkayMan') {
      win = (gambledPoints * 5);
    } else {
      win = (gambledPoints * 2);
    }
  ...
  if (win === 0) {
    ...
    ${username} lost ${gambledPoints} points in smp OMEGALUL`);
  }
  if (win > 0) {
