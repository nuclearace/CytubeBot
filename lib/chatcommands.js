...
CHAT_HANDLERS.set('logs', async (bot, username, msg) => {
  ...
  switch (fileName) {
    case logFileError:
      lines.push(...await errlog.read(numberOfLines));
      break;
    case logFileCytubeLog:
      lines.push(...await cytubelog.read(numberOfLines));
      break;
    case logFileSys:
      lines.push(...await syslog.read(numberOfLines));
      break;
    default:
      bot.sendPM(
          username,
          'Log file name not in switch condition. ' +
