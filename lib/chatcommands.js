...
import {filterMsg, genericUIDLoop, getCurrentUnixTimestamp, getUser, kill, parseBumpData, parseDeleteData, parseMediaLink, parseUserlimit, sendMessagesWithRateLimit, sendPmsWithRateLimit, sleep} from './utils.js';
...
CHAT_HANDLERS.set('cookie', async (bot, username, msg) => {
  {
    ...
    if (!userCookie.canClaimCookie()) {
      bot.sendChatMsg(
          `Can't claim now, wait ${userCookie.nextCookieAt.fromNow(/* withoutSuffix= */ true)}...`);
      return;
    }
  }
  const cookiesToClaim = Math.min(Math.round(geometric()), MAX_COOKIES_PER_CLAIM);
  const userCookie = await bot.cookies.claimCookie(username, cookiesToClaim);
  const claimedCookieType = COOKIE_TYPES[randomInt(COOKIE_TYPES.length)];

  bot.sendChatMsg(
      `${username} claimed ${cookiesToClaim} ${claimedCookieType} cookies, ` +
      `now has ${userCookie.count} cookies! ` +
...
CHAT_HANDLERS.set('cookies', async (bot, username, msg) => {
  ...
  bot.sendChatMsg(`${target} has ${userCookie.count} cookie${userCookie.count !== 1 ? 's' : ''}`);
});
