...
export function checkPermission(bot, data) {
  ...
  bot.db.getUserRank(data.username, (userRank) => {
    if (permission) {
      ...
      try {
        hybridPerm = userHasPermission(bot, permissionData).hasPermission;
      } catch (e) {
        hybridPerm = false;
      }
    }
...
export function handleHybridModPermissionChange(bot, data) {
  ...
  const hasPermission = userHasPermission(bot, permissionData);
...
// of the user.
export function userHasPermission(bot, permissionData) {
