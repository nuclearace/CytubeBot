const perms = {
  ...
  'checkPermission': (bot, data) => {
    ...
    bot.db.getUserRank(data['username'], (userRank) => {
      if (permission) {
        ...
        try {
          hybridPerm =
              perms.userHasPermission(bot, permissionData)['hasPermission'];
        } catch (e) {
          hybridPerm = false;
        }
      }
  ...
  'handleHybridModPermissionChange': (bot, data) => {
    let permission = '';
    ...
    const hasPermission = perms.userHasPermission(bot, permissionData);
    ...
    if (hasPermission['hasPermission']) {
      ...
      if (change === '-' && !hasPermission['hasAll']) {
        for (let i = 0; i < permissions.length; i++) {
          bot.stats['hybridMods'][name] =
              bot.stats['hybridMods'][name].replace(permissions[i], '');
        }
      }
    } else if (change === '+' && !hasPermission['hasAll']) {  // Don't add perms
                                                              // if user has ALL
      bot.stats['hybridMods'][name] += permission;
    }
  ...
  // of the user.
  'userHasPermission': (bot, permissionData) => {
