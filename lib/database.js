...
import {dbRun, dbRunStatement} from './utils.js';
...
export class Database {
  ...
  async updateTables() {
    ...
    await this.updateDbVersion(2, async () => {
      await dbRun(
          this.db,
          'ALTER TABLE users ADD suspended_until_unix INTEGER DEFAULT 0');
    });
  ...
  async updateDbVersion(newVersion, updateFunc) {
    ...
    cytubelog.log(`Updating db to version ${newVersion}`);
    await updateFunc();

    // Special case to create the version row if it doesn't exist
    if (dbVersion === 0) {
      await dbRun(
          this.db, `INSERT INTO version(key, value) VALUES ('dbversion', 1)`);
    }

    await dbRunStatement(this.db.prepare(
        `UPDATE version SET value = ? WHERE key = 'dbversion'`, [newVersion]));
  }
  ...
  async updateUserPoints(username, delta) {
    ...
    const newPoints =
        (currentPoints + delta) >= 0 ? (currentPoints + delta) : 0;
    const statement = this.db.prepare(
        'UPDATE users SET points = ? WHERE LOWER(uname) = ?',
        [newPoints, username.toLowerCase()]);
    return dbRunStatement(statement);
  }
  ...
  /**
   * Set the end time for a user's suspension.
   *
   * @param {string} user The user to time out.
   * @param {number} timestamp The Unix timestamp (seconds) when the user's
   *     timeout ends.
   * @return {!Promise<void>} Empty promise indicating the operation's
   *     completion.
   */
  setUserSuspension(user, timestamp) {
    const statement = this.db.prepare(
        'UPDATE users SET suspended_until_unix = ? WHERE uname = ?',
        [timestamp, user]);
    return dbRunStatement(statement);
  }
  ...
  async setModuleEnabled(module, enabled) {
    cytubelog.log(`Setting module ${module} to enabled=${enabled}`);
  ...
  async moduleIsEnabled(module) {
    const statement = this.db.prepare(
        'SELECT enabled FROM modules WHERE LOWER(name) = ?',
        [module.toLowerCase()]);
    ...
    return new Promise((resolve, reject) => {
      statement.get((err, row) => {
        ...
        if (typeof row === 'undefined') {
          errlog.log(`Module ${module} does not exist`);
          resolve(false);
          return;
        }
  ...
  /**
   * Get the end time for a user's suspension.
   *
   * @param {string} user The user to time out.
   * @return {!Promise<number>} The Unix timestamp (seconds) when the user's
   *     timeout ends.
   */
  getUserSuspensionEnd(user) {
    const statement = this.db.prepare(
        'SELECT suspended_until_unix FROM users WHERE uname = ?', [user]);
    return new Promise((resolve, reject) => {
      statement.get((err, row) => {
        if (typeof row !== 'undefined') {
  ...
  getUserPoints(username) {
    const statement = this.db.prepare(
        'SELECT points FROM users WHERE LOWER(uname) = ?',
  ...
  getVersion() {
    ...
    return new Promise((resolve, reject) => {
      statement.get((err, row) => {
        ...
        resolve(
            (row === undefined || row.value === undefined) ? null : row.value);
      });
    });
  }
