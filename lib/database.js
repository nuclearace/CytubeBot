...
Database.prototype.createTables = () => {
  this.db.serialize();
  this.db.run(
      'CREATE TABLE IF NOT EXISTS users(uname TEXT, blacklisted TEXT, block TEXT, primary key(uname))');
  this.db.run(
      'CREATE TABLE IF NOT EXISTS chat(timestamp INTEGER, username TEXT, msg TEXT, channel TEXT)');
  this.db.run(
      'CREATE TABLE IF NOT EXISTS ' +
      'videos(type TEXT, id TEXT, duration_ms INTEGER, title TEXT, flags INTEGER, primary key(type, id))');
  this.db.run(
      'CREATE TABLE IF NOT EXISTS video_stats(type TEXT, id TEXT, uname TEXT)');
  this.db.run(
      'CREATE TABLE IF NOT EXISTS user_count(timestamp INTEGER, count INTEGER, primary key(timestamp, count))');
  this.db.run(
      'CREATE TABLE IF NOT EXISTS version(key TEXT, value TEXT, PRIMARY KEY(key))');

  this.updateTables();
};

// Updates the tables as needed
Database.prototype.updateTables = () => {
  var self = this;
  this.getVersion(version => {
    if (!version) {
      var update = self.db.prepare(
          'INSERT INTO version(key, value) VALUES (?, ?)', ['dbversion', '1']);
      update.run(() => {
...
Database.prototype.flagVideo = (type, id, flags, title) => {
  this.logger.syslog.log(
      '*** Flagging video: ' + title + ' with flag: ' + flags);
...
// we deleted
Database.prototype.deleteVideos = (like, callback) => {
  var db = this;
  ...
  var deleteVideos = () => {
    for (var i = 0; i < videoIds.length; i++) {
      var stmt1 = db.db.prepare(
          'DELETE FROM videos WHERE id = ? ' +
              'AND type = ?',
          [videoIds[i]['id'], videoIds[i]['type']]);
      var stmt2 = db.db.prepare(
...
Database.prototype.insertChat = (msg, time, nick, room) => {
  var stmt = this.db.prepare(
...
// rank - The users rank
Database.prototype.insertUser = (username, rank) => {
  if (!username) {
    return;
  }
...
Database.prototype.insertUserBlacklist = (username, flag, callback) => {
  this.logger.syslog.log(
      'Setting blacklist: ' + flag + ' on user: ' + username);
...
// user - The user
// flag - The value
Database.prototype.insertUserBlock = (username, flag, callback) => {
  this.logger.syslog.log(
      '*** Setting block: ' + flag + ' on user: ' + username);
  var stmt = this.db.prepare(
...
Database.prototype.insertUserRank = (username, rank) => {
  var stmt = this.db.prepare(
...
Database.prototype.insertUsercount = (count, timestamp) => {
  var stmt = this.db.prepare(
...
// callback - The callback function
Database.prototype.getAllBlacklistedUsers = callback => {
  var stmt =
...
Database.prototype.getAllBlockedUsers = callback => {
...
// callback - The callback function
Database.prototype.getAverageUsers = callback => {
  var select_cls =
      'SELECT STRFTIME(\'%s\', STRFTIME(\'%Y-%m-%dT%H:00\', timestamp/1000, \'UNIXEPOCH\'))*1000 AS timestamp,' +
...
// callback - The callback function
Database.prototype.getChatStats = callback => {
...
// callback - The callback function
Database.prototype.getGeneralStats = callback => {
  var self = this;
  var stmt = 'ANALYZE';
  var stmt2 =
      'SELECT stat FROM sqlite_stat1 WHERE tbl = \'users\' OR tbl = \'videos\' OR tbl = \'chat\'';

  this.db.serialize(() => {
    self.db.run(stmt);
    self.db.all(stmt2, (err, rows) => {
      if (rows) {
        callback(rows);
      }
    });
...
// callback - The callback function
Database.prototype.getPopularVideos = callback => {
  var select_cls =
      'SELECT videos.type, videos.id, videos.title, videos.flags & 1, count(*) AS count FROM videos, video_stats';
  var where_cls =
      ' WHERE video_stats.type = videos.type AND video_stats.id = videos.id AND NOT videos.flags & 2 ';
  var group_cls =
      ' GROUP BY videos.type, videos.id ORDER BY count(*) DESC LIMIT 10';
  ...
  stmt.all((err, rows) => {
    ...
    for (var i = 0; i < rows.length; i++) {
      returnData.push([
        rows[i]['type'], rows[i]['id'], rows[i]['title'], rows[i]['flags'],
        rows[i]['count']
      ]);
    }
    callback(null, returnData);
...
// callback - The callback function
Database.prototype.getQuote = (nick, callback) => {
  nick = nick.split(' ')[0];
  var stmt = {};
  ...
  stmt =
      'SELECT username, msg, timestamp FROM chat WHERE msg NOT LIKE \'/me%\' ' +
...
// callback - The callback function
Database.prototype.getStats = (room, callback) => {
  var self = this;
...
// callback - The callback function
Database.prototype.getUserBlacklist = (username, callback) => {
  var stmt = this.db.prepare(
...
Database.prototype.getUserBlock = (username, callback) => {
  var stmt =
...
// Gets a user's rank
// Callback - callback function
Database.prototype.getUserRank = (username, callback) => {
  var stmt =
...
Database.prototype.getVersion = callback => {
  var stmt =
...
// callback - The callback function
Database.prototype.getVideos = (num, callback) => {
  if (!num) {
    num = 1;
  }

  var stmt = this.db.prepare(
      'SELECT type, id, duration_ms, title FROM videos ' +
...
Database.prototype.getVideosCountForClean = callback => {
  var self = this;
...
// callback - The callback function
Database.prototype.getVideosCount = callback => {
  var self = this;
...
// callback - The callback function
Database.prototype.getVideoStats = callback => {
  var select_cls =
      'SELECT uname, count(*) AS count FROM video_stats vs, videos v ';
  var where_cls =
      ' WHERE vs.type = v.type AND vs.id = v.id AND NOT v.flags & 2 ';
...
// callback - The callback function
Database.prototype.getVideoFlag = (type, id, callback) => {
  var stmt = this.db.prepare(
      'SELECT flags FROM videos videos WHERE type = ? AND id = ?', [type, id]);
