...
export class Database {
  ...
  async updateTables() {
    ...
    if (!version) {
      cytubelog.log('Updating db to version 1');

      await new Promise(
          (resolve, reject) => this.db.run(
              `INSERT INTO version(key, value) VALUES ('dbversion', '1')`,
              resolve));
      await new Promise(
          (resolve, reject) =>
              this.db.run('ALTER TABLE users ADD rank INTEGER', resolve));
      await new Promise((resolve, reject) => this.db.parallelize(resolve));
      version = '1';
    }
    const dbVersion = parseInt(version, 10);
    if (dbVersion < 2) {
      cytubelog.log('Updating db to version 2');
      await new Promise(
          (resolve, reject) => this.db.run(
              `UPDATE version SET value = 2 WHERE key = 'dbversion'`, resolve));
      await new Promise(
          (resolve, reject) => this.db.run(
  ...
  setUserSuspension(user, timestamp) {
    const statement = this.db.prepare(
        ...
        [timestamp, user]);
    return new Promise((resolve, reject) => statement.run(resolve));
  }
