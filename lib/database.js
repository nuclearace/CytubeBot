...
Database.prototype.getVideos = function(num, callback) {
    ...
    var stmt = this.db.prepare("SELECT type, id, duration_ms, title FROM videos " +
...
Database.prototype.getVideosCountForClean = function(callback) {
    var self = this
    
    this.db.get("SELECT count(*) AS count FROM videos WHERE flags = 0 "
        + "AND type = 'yt' AND duration_ms < ?", [this.maxVideoLength], function(err, row) {
        if (err) {
