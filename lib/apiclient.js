...
export function callAnagram(msg, callback) {
  const options = {
...
export function callWolfram(query, apiKey, callback) {
  const options = {
...
export function callWeather(data, apiKey, callback) {
...
export function callForecast(data, apiKey, callback) {
  if (data.split(' ').length === 1) {
    const options = {
      host: 'api.wunderground.com',
...
export function socketLookup(serverData, apiKeys, callback) {
  ...
  urlRetrieve((secure ? https : http), options, (res, data) => {
    ...
    // we should exit
    if (res !== 200) {
      errlog.log(`!~~~! Error looking up Cytube server info ${res}`);
      process.exit(1);
    }
...
export function youtubelookup(id, apiKey, callback) {
  const params = [
