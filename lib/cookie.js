...
export const COOKIE_CLAIM_COOLDOWN = moment.duration('2', 'hours');
...
export class Cookies {
  ...
  async getUserCookie(user) {
    ...
    const expireTime = moment.unix(expireTimestamp || 0);
    return new UserCookie(user, count, expireTime.add(COOKIE_CLAIM_COOLDOWN));
  }
}
...
class UserCookie {
   ...
   * @param {!moment.Moment} nextCookieAt When the next cookie can be claimed at.
   */
   ...
   */
  canClaimCookie() {
    return moment().isSameOrAfter(this.nextCookieAt);
  }
}
