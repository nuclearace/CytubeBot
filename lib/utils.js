...
export async function sendMessagesWithRateLimit(bot, messages) {
  ...
  if (messages.length > 20) {
    // For spams of >20 messages, the bot crashes unless we rate-limit it to ~120ms between
    // messages.
    waitTimeMs = 120;
  }
...
export function findVideosAddedByUser(bot, name) {
  ...
  return bot.playlist.filter((video) => video.queueby.toLowerCase() === name.toLowerCase())
      .map((video) => video.uid);
}
...
export function findVideosFromTitle(bot, title) {
  ...
  const regExEsc = (str) =>
      String(str).replace(/[\\\[\].()|{}$+*?!:^,#<-]/g, '\\$&').replace(/\x08/g, '\\x08');
  const reg = new RegExp('.*' + regExEsc(title) + '.*', 'ig');

  return bot.playlist.filter((video) => video.media.title.match(reg)).map((video) => video.uid);
}
...
export function genericUIDLoop(bot, data) {
  ...
  if (!bot[kind]) {
    errorLog.log(`!~~~! genericUIDLoop No such method: ${kind}`);
    return;
  }
 ...
 * @return {{kind: string, num: number, uids: !Array<string>}} An object containing the number to
 *     bump and the uids.
 */
 ...
 *
 * @param {CytubeBot} bot Reference to the CytubeBot.
 * @param {string} username User requesting the delete.
 * @param {string} msg Message the user sent requesting delete.
 * @return {{kind: string, name: string, num: number, uids: !Array<string>}} An object containing
 *     items needed for the generic uid loop.
 */
export function parseDeleteData(bot, username, msg) {
  const userData = msg.split(' ');
  let name = '';
  let num = 0;
...
export function parseForecastData(parsedJSON, tomorrow) {
  ...
  if (tomorrow) {
    if ((location.split(', ')[1]).length != 2) {
      ...
      returnStrings.push(`Tomorrow Night: ${forecast.tomorrowNight.fcttext_metric}`);
    } else {
      returnStrings.push(
          `Location: ${location} ` +
    ...
    if ((location.split(', ')[1]).length != 2) {
      returnStrings.push(`Location: ${location} Today: ${forecast.todayDay.fcttext_metric}`);
      returnStrings.push(`Tonight: ${forecast.todayNight.fcttext_metric}`);
    } else {
      returnStrings.push(`Location: ${location} Today: ${forecast.todayDay.fcttext}`);
