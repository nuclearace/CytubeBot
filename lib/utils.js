...
export function findVideosAddedByUser(bot, name) {
  ...
  return bot.playlist
      .filter((item) => item.queueby.toLowerCase() === name.toLowerCase())
      .map((item) => item.uid);
}
...
export function findVideosFromTitle(bot, title) {
  ...
  return bot.playlist.filter((item) => item.media.title.match(reg))
      .map((item) => item.uid);
}
 ...
 */
export function parseUserlimit(bot, match, callback) {
  let isTrue = true;
  let isFalse = false;
  let num = 0;
