var utilHandlers = {
	...
	// Bump videos either by user or titles
	// bot - Reference to the bot
	// bumpData - The bump data
	// bumpData is meant to be an object with members:
	// kind: either "-user" or "-title"
	// num: number or "all"
	// uids: the uids of the videos
	"bumpVideos": function(bot, bumpData) {
		if (!bumpData)
			return

		var bumpKind = bumpData["kind"]
		var num = bumpData["num"]
		var uids = bumpData["uids"]

		if (!num) {
			// We should move the last match
			bot.sendMoveMedia(uids[0])
		} else if (num === "all") {
			// We should move all the videos matched
			for (var i = 0; i < uids.length; i++) {
				bot.sendMoveMedia(uids[i])
			}
		} else {
			// We should move num videos matched
			for (var i = 0; i < num; i++) {
				if (i > uids.length)
					break
				bot.sendMoveMedia(uids[i])
