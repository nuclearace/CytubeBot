...
export class Gamba {
  ...
  startWatchingForDuelExpiration() {
    ...
    setInterval(() => {
      this.pendingDuels = this.pendingDuels.filter((duel) => moment().isBefore(duel.expireTime));
...
export class PendingDuel {
  static EXPIRE_AFTER = moment.duration(30, 'seconds');
  ...
  constructor(initiator, target, amount) {
    ...
    this.target = target;
    this.amount = amount;
    this.expireTime = moment().add(PendingDuel.EXPIRE_AFTER);
  }
}
