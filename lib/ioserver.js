...
function IOServer(srv, bot) {
  ...
  io.sockets.on('connection', socket => {
    ...
    socket.on('getRoom', () => {
      ioServer.getRoom(room => {
        socket.emit('room', room);
      });
    });

    socket.on('getStats', () => {
      ioServer.getStats(data => {
...
IOServer.prototype.getEmotes = callback => {
  callback(this.bot.channelEmotes);
};

IOServer.prototype.getRoom = callback => {
  callback(this.bot.room);
};

IOServer.prototype.getStats = callback => {
  this.bot.getStats(data => {
    callback(data);
  });
};

IOServer.prototype.handleInternals = socket => {
