...
CytubeBot.prototype.addVideo = function(type, id, duration, temp, parsedLink) {
	...
	if (!parsedLink) {
		...
		console.log("!~~~! Sending queue frame for " + json["id"])
...
CytubeBot.prototype.checkPermission = function(username, rank, permission) {
	...
	if (rank) {
		try {
			...
			console.log("!~~~!: Error looking up rank")
		}
	}
...
CytubeBot.prototype.getSocketURL = function(server) {
	...
	if (server.match(defaultReg)) {
		console.log("!~~~! Found socketIO info in config")
		return io.connect(server)
	} else {
		console.log("!~~~! Looking up socketIO info from server")
...
CytubeBot.prototype.validateVideo = function(video, callback) {
	...
	var postValidate = function(shouldDelete, why) {
		...
		switch (why) {
			case "disabled":
				console.log("!~~! Emedding disabled: " + id)
				bot.sendPM(nick, "Embedding disabled: " + id)
				break

			case "blocked":
				console.log("!~~~! Video blocked in: " + bot.deleteIfBlockedIn)
