...
CytubeBot.prototype.addHandlers = function() {
	...
	bot.socket.on("connect", function() {
		bot.logger.syslog.log("!~~~! Connected to server")
		bot.start()
	})
	...
	bot.socket.on("disconnect", function() {
		bot.logger.syslog.log("!~~! Disconnected from the server")
		setTimeout(function() {
			process.exit(0)
		}, 10000)
	})
...
CytubeBot.prototype.checkPermission = function(username, rank, permission) {
	...
	return perms.handle(this, "checkPermission", permData)
};
...
CytubeBot.prototype.getSocketURL = function(server) {
	...
	if (server.match(defaultReg)) {
		...
		return require('socket.io-client')(server, {transports: ['websocket']})
	} else {
		...
		api.APICall(server, "socketlookup", null, function(data) {
			if (data.match(defaultReg)) {
				bot.socket = require('socket.io-client')(data, {transports: ['websocket']})
				bot.addHandlers()
			}
		})
	}
