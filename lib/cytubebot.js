...
export class CytubeBot {
  ...
  async init(config) {
    ...
    this.watchForTimeoutExpirations();
  ...
  /**
   * Watch for timeouts expiring, and un-timeout users when their timeout expires.
   */
  watchForTimeoutExpirations() {
    ...
    setInterval(async () => {
      ...
      const usersToUnmute = this.userlist
                                .filter(
                                    (user) => user.meta.muted &&
                                        !timedOutUsers.includes(user.name) && !user.meta.smuted)
                                .map((user) => user.name);
