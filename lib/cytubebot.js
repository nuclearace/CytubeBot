...
CytubeBot.prototype.disallowUser = function(user, disallow) {
	...
	if (disallow && !(user in this.stats["disallow"])) {
		this.logger.syslog.log("!~~~! Disallowing: " + user)
		this.stats["disallow"][user] = user
	} else if (user in this.stats["disallow"] && !disallow) {
		this.logger.syslog.log("!~~~! Allowing: " + user)
		delete this.stats["disallow"][user]
	}
...
CytubeBot.prototype.handleChatMsg = function(data, pm) {
	...
	var allowed = function() {
		if (!(username.toLowerCase() in bot.stats["disallow"]))
			return true
		else {
