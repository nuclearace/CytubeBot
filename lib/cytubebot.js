...
export class CytubeBot {
  ...
  handleChangeMedia(data) {
    if (this.stats.managing && this.loggedIn && !this.firstChangeMedia &&
      ...
      try {
        ...
        cytubelog.log('!~~~! Media deleted. handleChangeMedia lookup temp failed');
      }
  ...
  async kickUserIfSuspended(user) {
    ...
    cytubelog.log(`Kicking ${user} as they are suspended for another ${remaining}`);
    this.sendPm(user, `You are suspended from ${this.room} for another ${remaining}`);
