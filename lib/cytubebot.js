...
export class CytubeBot {
  ...
  async handleAddMedia(data) {
    ...
    // Move new user-queued songs above the bot-queued songs (from !addrandom)
    if (!this.stats.managing) {
      if (data.after !== this.playlist.length - 2) {
      ...
      const firstBotVideoIndex = this.playlist.indexOf(firstBotVideo);
      const firstNonBotVideo = this.playlist[firstBotVideoIndex - 1];

      this.sendMoveMedia(data.item.uid, firstNonBotVideo.uid);
    }
  }
