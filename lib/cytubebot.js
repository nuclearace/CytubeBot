...
export class CytubeBot {
  ...
  addRandomVideos(num) {
    const postGet = (video) => {
      ...
      const id = video['id'];
      const duration = video['duration_ms'] / 1000;

      this.addVideo(type, id, duration);
    };
  ...
  blacklistUser(username, flag) {
    ...
    if (flag) {
      const uids = utilsHandle(this, 'findVideosAddedByUser', username);
  ...
  blacklistVideo(uid, callback) {
    ...
    if (typeof uid !== 'undefined') {
      const video = utilsHandle(this, 'getVideoFromUID', uid);
  ...
  checkPermission(username, rank, permission, callback) {
    ...
    permissionsHandle(this, 'checkPermission', permData);
  }
      ...
      this.db.getVideos(num, (videos) => {
        ...
        videos.forEach((video) => {
          ...
          this.videoLookupLimiter.removeTokens(1, () => {
            count++;
            validate(this, video.id, video.type, video.title, postValidate);
          });
        });
      });
    });
  ...
  countVideosAddedByUser(username) {
    if (!username) {
      for (let i = 0; i < this.userlist.length; i++) {
        const uids = utilsHandle(
            this, 'findVideosAddedByUser', this.userlist[i]['name']);
        this.userlist[i]['addedMedia'] = uids;
      }
      return;
    }
    const pos = utilsHandle(this, 'findUser', username);
    this.userlist[pos]['addedMedia'] =
        utilsHandle(this, 'findVideosAddedByUser', username);
  }
  ...
  getQuote(nick) {
    this.db.getQuote(nick, (row) => {
      ...
      msg = utilsHandle(this, 'filterMsg', msg);
  ...
  getSocketURL(server) {
    ...
    apiCall(serverData, 'socketlookup', null, (data) => {
  ...
  handleAddMedia(data) {
    ...
    const pos = utilsHandle(this, 'findUser', data['item']['queueby']);
    if (typeof pos !== 'undefined') {
      if (this.stats['userLimit'] &&
    ...
    if (this.playlist.length === 0) {
      ...
      const index = utilsHandle(this, 'findIndexOfVideoFromUID', uid);
  ...
  // data - addUser data
  handleAddUser(data) {
    const inList = utilsHandle(this, 'userInUserlist', data['name']);
  ...
  handleBanlist(data) {
    ...
    utilsHandle(this, 'loopThroughWaiting', 'unban');
  }
  ...
  handleChangeMedia(data) {
    if (this.stats['managing'] && this.loggedIn && !this.firstChangeMedia &&
      ...
      try {
        if (typeof uid !== 'undefined') {
          temp = utilsHandle(this, 'getVideoFromUID', uid)['temp'];
        }
      } catch (e) {
        cytubelog.log(
  ...
  handleChatMsg(data, pm) {
    ...
    const handleCommand = msg.indexOf('$') === 0 &&
        username.toLowerCase() !== this.username.toLowerCase() &&
        this.loggedIn && allowed();

    if (handleCommand) {
      return chatHandle(this, username, msg);
    }
  ...
  handleDeleteMedia(data) {
    ...
    if (typeof index !== 'undefined') {
      ...
      const addedBy = utilsHandle(this, 'getVideoFromUID', uid)['queueby'];
      const pos = utilsHandle(this, 'findUser', addedBy);
  ...
  handleHybridModPermissionChange(permission, name) {
    ...
    permissionsHandle(this, 'handleHybridModPermissionChange', permData);
  }
  ...
  handleLogin(data) {
    if (!data['success']) {
      return syslog.log('!~~~! Failed to login');
    }
  ...
  handleMoveMedia(data) {
    ...
    const fromIndex = utilsHandle(this, 'findIndexOfVideoFromUID', from);

    // Remove video
    const removedVideo = this.playlist.splice(fromIndex, 1);
    const afterIndex = utilsHandle(this, 'findIndexOfVideoFromUID', after);
  ...
  handleSetLeader(name) {
    if (name.toLowerCase() === this.username.toLowerCase()) {
      this.isLeader = true;
      utilsHandle(this, 'loopThroughWaiting', 'settime');
    } else {
      this.isLeader = false;
    }
  }
  ...
  handleSetTemp(data) {
    ...
    if (typeof index === 'undefined') {
      return syslog.log('Error: handleSetTemp.index undefined.');
    }
  ...
  handleUserLeave(user) {
    const index = utilsHandle(this, 'findUser', user);
    if (typeof index !== 'undefined') {
  ...
  recordEmoteCombo(username, msg) {
    ...
    if (words.every((word) => !emoteNames.includes(word))) {
      // Message contains no emotes (WutFace)
  ...
  sendAssignLeader(user) {
    ...
    try {
      rank = utilsHandle(this, 'getUser', this.username)['rank'];
    } catch (e) {
    }  // Not in list
  ...
  sendChatMsg(message, override) {
    ...
    rank = utilsHandle(this, 'getUser', this.username.toLowerCase());
  ...
  validateVideo(video, callback) {
    ...
    try {
      rank = utilsHandle(this, 'getUser', nick)['rank'];
    } catch (e) {
      rank = 0;
    }
    ...
    const postValidate = (shouldDelete, why) => {
      ...
      switch (why) {
        ...
        case 'blocked':
          ...
          this.sendPM(
              nick,
              'Video blocked in: ' + this.deleteIfBlockedIn + '. id: ' + id);
          break;
    ...
    const postUserBlock = (block) => {
      ...
      if (type === 'yt' && this.youtubeapi) {
        validate(this, id, type, title, postValidate);
      } else {
