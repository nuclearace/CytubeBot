...
function CytubeBot(config) {
	...
	this.db = Database.init(this.logger)
...
CytubeBot.prototype.addHandlers = function() {
	...
	bot.socket.on("pm", function(data) {
		var rank = utils.handle(bot, "getUser", data["username"])["rank"]
		if (rank < 2)
			return
...
CytubeBot.prototype.checkPermission = function(username, rank, permission) {
	var userRank = 0
	var hybridPerm = false

	if (rank) {
		try {
			userRank = utils.handle(this, "getUser", username)["rank"]
		} catch (error) {
			this.logger.errlog.log("!~~~!: Error looking up rank: CytubeBot.checkPermission")
		}
	}
	...
	if (permission) {
		...
		try {
			hybridPerm = utils.handle(this, "userHasPermission", permissionData)["hasPermission"]
		} catch (e) {
			hybridPerm = false
		}
	}
...
CytubeBot.prototype.handleAddUser = function(data) {
	...
	if (!inList) {
		...
		this.logger.cytubelog.log("Added User: " + data["name"])
		this.logger.cytubelog.log("Userlist has : " + this.userlist.length + " users")
...
CytubeBot.prototype.handleHybridModPermissionChange = function(permission, name) {
	if (!permission)
		return this.sendHybridModPermissions(name)
...
CytubeBot.prototype.handleMediaUpdate = function(data) {
	this.logger.cytubelog.log("### Current video time: " + data["currentTime"] + " Paused: " + data["paused"])
...
CytubeBot.prototype.handleUserLeave = function(user) {
	...
	if (index) {
		...
		this.logger.cytubelog.log("!~~~! Removed user: " + user)
		this.logger.cytubelog.log("!~~~! Userlist has : " + this.userlist.length + " users")
	}
};
...
CytubeBot.prototype.sendChatMsg = function(message) {
	...
	if (this.stats["muted"] || !this.doneInit)
		return
...
CytubeBot.prototype.start = function() {
	...
	setTimeout(function() {
		bot.logger.syslog.log("!~~~! Now handling commands")
		bot.doneInit = true
		bot.readTimes(function(data) {
...
CytubeBot.prototype.validateVideo = function(video, callback) {
	...
	var postValidate = function(shouldDelete, why) {
		...
		switch (why) {
			case "disabled":
				this.logger.syslog.log("!~~! Emedding disabled: " + id)
				bot.sendPM(nick, "Embedding disabled: " + id)
				break

			case "blocked":
				this.logger.syslog.log("!~~~! Video blocked in: " + bot.deleteIfBlockedIn)
				bot.sendPM(nick, "Video blocked in: " + bot.deleteIfBlockedIn +
					". id: " + id)
				break

			case "invalid":
				this.logger.syslog.log("!~~~! Invalid video: " + id)
