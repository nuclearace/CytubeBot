...
export class CytubeBot {
  ...
  blacklistVideo(uid) {
    const flags = 1;
    let type = this.currentMedia.type;
   ...
   */
  async checkUserBlacklist(username) {
    if (!username) {
      return false;
    }
  ...
  async cleanDatabaseVideos() {
    ...
    for (const video of videos) {
      ...
      validateYouTubeVideo(
          this, video.id, video.type, video.title, (shouldDelete, why) => {
            if (shouldDelete) {
              numFlagged++;
            }
  ...
  grantAutomaticUserPoints(activePoints = 0, afkPoints = 0) {
    ...
    for (const user of this.userlist) {
      if (user.meta.afk) {
        this.db.updateUserPoints(user.name, afkPoints);
      } else {
        this.db.updateUserPoints(user.name, activePoints);
      }
    }
  }
  ...
  handlePlaylist(playlist) {
    ...
    for (const u of playlist) {
      this.validateVideo(u, (block, uid) => {
        if (block) {
          this.deleteVideo(uid);
        }
      });
   ...
   *     a deleteVideo.
   */
  async validateVideo(video, callback) {
    ...
    const postValidate = async (shouldDelete, why) => {
      if (!shouldDelete) {
        postUserBlacklist(await this.checkUserBlacklist(nick));
        return;
      }
    ...
    if (type === 'yt' && this.youtubeapi) {
      ...
      const blacklist = await this.checkUserBlacklist(nick);
      if (blacklist) {
        this.blacklistVideo(uid);
      }
    }
