...
export class CytubeBot {
  ...
  async init(config) {
    ...
    /** @type {string} */
    this.weatherUndergroundApiKey = config.weatherunderground;
    /** @type {string} */
     ...
     * @type {RateLimiter}
     */
    this.weatherLimiter = new RateLimiter({tokensPerInterval: 10, interval: 'minute'});
    /** @type {TokenBucket} */
    ...
    /** @type {RateLimiter} */
    this.cleverbotLimiter = new RateLimiter({tokensPerInterval: 10000 / 31, interval: 'day'});
    /** @type {RateLimiter} */
    ...
    /** @type {RateLimiter} */
    this.pyramidLimiter = new RateLimiter({tokensPerInterval: 1, interval: 15 * 1000});
    /** @type {RateLimiter} */
  ...
  handleSetUserMeta(event) {
    infoLog.log(`received userlist for ${event.name}`);
   ...
   */
  monitorTwitchChannels() {
    const checkIntervalSeconds = 5;
    setInterval(async () => {
      for (const channel of this.monitoredTwitchChannels) {
        const live = await this.twitch.isChannelLive(channel.name);

        // Wasn't live. Now is.
        if (!channel.live && live && this.firstTwitchChannelMonitorDone) {
