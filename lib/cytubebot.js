...
class CytubeBot {
  ...
  blacklistUser(username, flag) {
    ...
    if (flag) {
      const uids = utils.handle(this, 'findVideosAddedByUser', username);
  ...
  blacklistVideo(uid, callback) {
    ...
    if (typeof uid !== 'undefined') {
      const video = utils.handle(this, 'getVideoFromUID', uid);
  ...
  blockUser(username, flag) {
    ...
    if (flag) {
      ...
      for (const uid of uids) {
        this.blacklistVideo(uid, () => bot.deleteVideo(uid));
      }
    }
  }
  ...
  checkPlaylist() {
    ...
    for (let i = 0; i < this.userlist.length; i++) {
      if (this.userlist[i]['addedMedia'].length > this.stats['userLimitNum']) {
        ...
        const uids = bot.userlist[i]['addedMedia'].reverse();
  ...
  handleAddMedia(data) {
    ...
    const pos = utils.handle(this, 'findUser', data['item']['queueby']);
    if (typeof pos !== 'undefined') {
      if (this.stats['userLimit'] &&
    ...
    if (this.playlist.length === 0) {
      ...
      const index = utils.handle(this, 'findIndexOfVideoFromUID', uid);
  ...
  // data - addUser data
  handleAddUser(data) {
    const inList = utils.handle(this, 'userInUserlist', data['name']);
  ...
  handleDeleteMedia(data) {
    ...
    if (typeof index !== 'undefined') {
      ...
      const addedBy = utils.handle(this, 'getVideoFromUID', uid)['queueby'];
      const pos = utils.handle(this, 'findUser', addedBy);
  ...
  handleMoveMedia(data) {
    ...
    const fromIndex = utils.handle(this, 'findIndexOfVideoFromUID', from);

    // Remove video
    const removedVideo = this.playlist.splice(fromIndex, 1);
    const afterIndex = utils.handle(this, 'findIndexOfVideoFromUID', after);
  ...
  handleUserLeave(user) {
    const index = utils.handle(this, 'findUser', user);
    if (typeof index !== 'undefined') {
