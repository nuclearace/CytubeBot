...
function CytubeBot(config) {
	...
	this.squeeLimiter = new RateLimiter(1, "minute")
...
CytubeBot.prototype.addRandomVideos = function(num) {
	...
	this.db.getVideos(num, function(rows) {
		...
		// Add each video, using a rate limiter
		rows.forEach(function(video) {
			bot.addVideoLimiter.removeTokens(1, function(err, tokensRemaining) {
				delayed(video)
			})
		})
	})
	...
	this.db.getVideoFlag(type, id, function(row) {
		...
		bot.checkUserBlock(nick, function(block) {
			...
			if (type === "yt" && bot.youtubeapi) {
				validator.validate(bot, id, type, title, function(shouldDelete) {
					if (shouldDelete) {
						return callback(true, uid)
					}
				})
